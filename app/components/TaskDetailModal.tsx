...
type TaskDetailProps = {
  ...
  onUpdatePlatforms?: (id: number, platforms: Platform[]) => void;
};
...
export default function TaskDetailModal({
  ...
  const [selectedPlatforms, setSelectedPlatforms] = useState<Platform[]>(task.platforms ?? []);
      ...
      >
        <div className="flex flex-col p-6 gap-y-8">
          ...
          {editingField === "title" ? (
            ...
            <h2
              className="text-2xl font-light border-b border-transparent cursor-pointer"
          ...
          <div className="flex-col">
            ...
            >
              ...
              {editingField === "due_date" ? (
                ...
                <p className="py-4 border-b border-transparent">
                  {task.due_date ?? "Add Due Date"}
                </p>
              )}
            </div>
          ...
          {editingField === "platforms" ? (
            ...
            <PlatformSelector
              ...
              onChange={(selected) => {
                ...
                onUpdatePlatforms?.(task.id!, selected);
              }}
            ...
            <div
              ...
              onClick={() => setEditingField("platforms")}
            >
              {selectedPlatforms.length > 0 ? (
                selectedPlatforms.map((p) => (
                  <span
                    key={p.id}
                    className="bg-blue-900 text-blue-200 font-bold rounded-full px-2 py-0.5 text-sm"
                  >
                    {p.title}
                  </span>
                ))
              ) : (
                <p className="">Select Platforms</p>
              )}
            </div>
          )}
        </div>
